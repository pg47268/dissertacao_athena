...
class WalkFunction:
    ...
    def trajectory(self, id, t, contact):
        ...
        if 1.0 <= t < 1.2 and self.t_swing != 1:
            ...
            self.t_swing = 1
        elif 2.0 <= t <= 2.1 and self.t_stance != 1:
...
class Athena:
    ...
    def posture(self, iteration):
        ...
        if len(sl) > 3:
            ...
            ge = sl
        print("ge, iteration ", ge, iteration)
        if len(ge) == 3:
            feet_O = {}
            h_relative = []
            joints = real_to_sim(self.dictionary(self.arduino.encoders))
...
if __name__ == '__main__':
    ...
    for i in range(18):
        ...
        print('FEITO ', i)
    print(hexapod.walker.tempo)
    print("altura ref", hexapod.h_ref)
    # Write the sensor data to a CSV file
    with open('/home/isabel/Desktop/test_athena_ard/new/reg_irreg_7.csv', 'w') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['vel_x', 'accel_x', 'ciclos', 'altura'])
        #print(len(hexapod.vx)," ", len(hexapod.ax), " ", len(hexapod.walker.tempo))
        for k in range(len(hexapod.vx)):
                writer.writerow([hexapod.vx[k], hexapod.ax[k], hexapod.walker.tempo[k], hexapod.desvio[k]])
                #writer.writerow([hexapod.vx[k], hexapod.ax[k], 0, hexapod.desvio[k]])
    #------------------------------------------------
    print(len(hexapod.walker.all_trajectory_foot1), len(hexapod.all_forces))
    print("d_h: ", hexapod.delta_h)
    print("p14 ", hexapod.walker.p_14)
    print("z_t ", hexapod.walker.z_t)
    print("fk : ", hexapod.foot_c)
    print(hexapod.time)
    ...
    with open('/home/isabel/Desktop/test_athena_ard/new/forca_reg_irreg_7.csv', 'w') as csvfile:
        writer = csv.writer(csvfile)
