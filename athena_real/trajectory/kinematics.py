...
class Kinematics:
    ...
    def fk(self, arg):
        return np.array([np.cos(self.tau[self._id] + arg[0]) * (self.l3 * np.cos(arg[1] + arg[2]) + self.l2 * np.cos(arg[1]) + self.l1) + self.a * np.cos(self.tau[self._id]), np.sin(self.tau[self._id] + arg[0]) * (self.l3 * np.cos(arg[1] + arg[2]) + self.l2 * np.cos(arg[1]) + self.l1) + self.a * np.sin(self.tau[self._id]), self.l3 * np.sin(arg[1] + arg[2]) + self.l2 * np.sin(arg[1])])

    def fk_tc(self, arg):
        return np.array([[np.cos(arg[0]) * (self.l3 * np.cos(arg[1] + arg[2]) + self.l2 * np.cos(arg[1]) + self.l1), np.sin(arg[0]) * (self.l3 * np.cos(arg[1] + arg[2]) + self.l2 * np.cos(arg[1]) + self.l1), self.l3 * np.sin(arg[1] + arg[2]) + self.l2 * np.sin(arg[1])]])
