...
class Env:
    ...
    def move_joints(self, i):
        ...
        it = i
        for x in range(3):
            if 0.2 < self.agent.sonar[x] < 0.4:
                self.obst_dinamico = True
    
            if 0.2 < self.agent.sonar[x] <= 0.6 and self.detect != 1:
                self.obstaculo += 1
                self.detect = 1
    ...
    try:
        ...
        for i in range(30):
            ...
            if hexapod.obst_dinamico == True:
                print("OBSTÀCULO PRÓXIMO") 
                hexapod.obst_dinamico = False       

            if hexapod.obstaculo >= 4:
                print("OBSTÁCULO")
            hexapod.detect = 0
        ...
        with open('/home/isabel/Desktop/tese/classificacao/class_ind_2/teste_obst.csv', 'w') as csvfile:
